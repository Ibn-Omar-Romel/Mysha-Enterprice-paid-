...
// TanStack Query v5 marks `queryKey` as required on `UseQueryOptions`, but the
// generated hooks below supply a default queryKey internally — so callers pass
// only things like `{ enabled }`. We relax the option type to Partial here; the
// hooks' return values still intersect `{ queryKey: QueryKey }`, so nothing that
// genuinely needs a queryKey loses it. (Single, safe override for the whole file.)
type UseQueryOptions<
  TQueryFnData = unknown,
  TError = unknown,
  TData = TQueryFnData,
> = Partial<RQUseQueryOptions<TQueryFnData, TError, TData>>;
